...
export function ProductImage({ product, className = "" }: { product: Product; className?: string }) {
  if (product.image_url) {
    ...
    return <img src={product.image_url} alt={product.title} className={`object-cover object-center ${className}`} />;
  }
  return (
    <div
...
export default function ProductTile({
  ...
  const imageHeight = compact ? "h-16" : fluid ? "aspect-square" : "h-24";
  return (
    <div
    ...
    >
      ...
      >
        <div className={`relative overflow-hidden bg-(--well) ${imageHeight}`}>
