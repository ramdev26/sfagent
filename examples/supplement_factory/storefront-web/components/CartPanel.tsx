    ...
    <BagPanel
      ...
      empty={
        <>
          ...
          <br />
          Ask Supp Factory Assistant for anything in the store.
        </>
      }
      footer={
        <>
          <TotalRow
            label={count ? `Subtotal · ${plural(count, "item")}` : "Subtotal"}
            value={formatMoney(cart?.subtotal ?? 0, cart?.currency)}
            note={items.length ? "Next-day island-wide when ordered before 12PM" : undefined}
          />
