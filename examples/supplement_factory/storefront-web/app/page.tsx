...
const ASSISTANT = "Supp Factory Assistant";

function Wordmark() {
  return (
    <span className="flex min-w-0 items-center gap-2.5 pr-1">
      <span aria-hidden className="grid h-[30px] w-[30px] shrink-0 place-items-center rounded-lg bg-(--ink) text-[13px] font-bold text-(--surface)">
        SF
      </span>
      <span className="truncate text-[16px] font-bold tracking-[-0.02em] text-(--ink) sm:text-[17px]">
        <span className="sm:hidden">Supp Factory</span>
        <span className="hidden sm:inline">Supplement Factory</span>
      </span>
    </span>
  );
}
...
export default function StorefrontPage() {
  ...
  return (
    <StoreShell
      ...
      placeholder={view === "orders" ? "Ask about an order number, a return, delivery…" : "Ask about a supplement, a brand, an order…"}
    >
